...
// Use the existing Greenstone Resend audience already wired into Netlify env.
// Pete will broadcast the "we're live" email to this audience when the site
// is unlocked. Override via RESEND_RELAUNCH_AUDIENCE_ID if a separate list is
// created later.
const AUDIENCE_ID =
...
export async function POST(req: NextRequest) {
  ...
  // Add to audience (idempotent — duplicates are fine)
  if (AUDIENCE_ID) {
    try {
    ...
    console.warn('[notify-relaunch] no AUDIENCE_ID — captured email but not added to a list');
  }
