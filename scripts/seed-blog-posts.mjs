...
const author = {
  ...
  bio: 'The Greenstone Peptides editorial team includes licensed pharmacists and clinical researchers with over 25 years of pharmaceutical compounding experience.',
};
...
const posts = [
  // ── POST 1 ─────────────────────────────────────────────────────────────────
  {
    ...
    body: [
      ...
      p('b15',
        'Most compounded BPC-157 is dispensed as a lyophilized powder that is reconstituted with bacteriostatic water before subcutaneous or intramuscular injection. Protocols vary by indication and should always be determined by a prescribing clinician. Once reconstituted, BPC-157 should be refrigerated and used within the stability window documented on the pharmacy&rsquo;s beyond-use date. Greenstone Peptides ships cold-chain to preserve the integrity of every compound.'),
  ...
  // ── POST 3 ─────────────────────────────────────────────────────────────────
  {
    ...
    body: [
      ...
      p('b15',
        'A legitimate peptide source is a licensed compounding pharmacy that dispenses against a valid prescription from a licensed clinician. It is inspected by a state board of pharmacy. It provides real, batch-specific Certificates of Analysis from accredited analytical labs. It ships under cold-chain where stability requires it. It names the pharmacist in charge. If a website offers to ship peptides directly to your door without a prescription, without a pharmacy license, and without traceable testing, it is not a legitimate source regardless of how professional the website looks.'),
      h2('b16', 'The Bottom Line'),
      p('b17',
        'The overseas peptide market is not a bargain. It is a transfer of risk from the seller to the buyer, disguised as a discount. For peptides injected into the body, the only defensible sourcing model is a licensed USA compounding pharmacy operating under USP 797 standards, dispensing against a legitimate prescription, with batch-specific testing on every lot. Greenstone Peptides was built on that model because we believe it is the only model that belongs in a vial that enters a human being.'),
    ],
  },
  ...
  // ── POST 7 ─────────────────────────────────────────────────────────────────
  {
    ...
    body: [
      ...
      p('b11',
        'The 2026 enforcement action made something that was already true legally and clinically much more practically obvious: the only stable long-term source for injectable peptides is a licensed compounding pharmacy operating against a valid prescription. A pharmacy pathway is not a workaround. It is the actual regulatory structure the FDA recognizes. It requires a prescribing clinician who has evaluated the patient, a pharmacist who fills the prescription under USP 797 standards, and documentation at every step of the process. It cannot be shut down by a fifteen-day warning letter because it is not operating in a gray zone.'),
